...
use sp_runtime::{
	...
	traits::{
		...
		OpaqueKeys, StaticLookup, SaturatedConversion, ConvertInto, DispatchInfoOf, Dispatchable,
		PostDispatchInfoOf,
	},
...
pub use frame_support::{
	...
	traits::{
		...
		U128CurrencyToVote, Contains, AsEnsureOriginWithArg, WithdrawReasons,
	},
	weights::{
		constants::{
...
use pallet_ethereum::{PostLogContent, EthereumBlockHashMapping};
...
//Fees Calcs
fn hex_to_array32(hex_string: &str) -> [u8; 32] {
    let s = &hex_string[2..];
    let vec: Vec<u8> = (0..s.len())
        .step_by(2)
        .map(|i| u8::from_str_radix(&s[i..i + 2], 16).expect("Failed to parse hex"))
        .collect();
...
fn distribute_fee(fee: Balance) {
    let split_node_wallet: AccountId = hex_to_array32("5EJ15atZqnEPbahueXpKun6yAtG1dYLWgEg1AB1b8sdXDBsx").into();
    let lunes_community_wallet: AccountId = hex_to_array32("6EJ15atZqfas54fa1s6f6fa958f4sa6aa8wf81b8sdXDBsx").into();
    let burn_wallet: AccountId = hex_to_array32("0000000000000000000000000000000000000").into();
    let split_node_share = (fee * 75) / 100;
    let lunes_community_share = (fee * 125) / 1000;
    let burn_share = (fee * 125) / 1000;
}
...
// Money matters for calculating collection deposits - NFT
...
impl pallet_evm::Config for Runtime {
	...
	type OnCreate = ();
	type Timestamp = Timestamp; // pode usar o pallet timestamp
	type WeightInfo = ();
